...
try {
 ...
 $('notice').textContent=data.mode==='sample'?'가상 예시 데이터 · 아래 수치·그래프·상태 분류는 화면 확인용이며 실제 수출 통계가 아닙니다.':'산업통상부 수출입동향 · 최신 13개월은 공식 PDF, 이전 기간은 참고 사이트의 월별 수치 표입니다. 잠정치 및 수정치가 포함됩니다.';
 $('source').textContent=data.mode==='sample'?'화면 확인용 가상 데이터 · KOSIS 연결 전':`출처: 산업통상부 / 모소밤부 월별표 · 수집: ${data.updatedAt?.slice(0,10) || '확인 필요'}`;
