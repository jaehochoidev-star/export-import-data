...
try {
 ...
 $('notice').textContent=data.mode==='sample'?'가상 예시 데이터 · 아래 수치·그래프·상태 분류는 화면 확인용이며 실제 수출 통계가 아닙니다.':'누적 월별 수출 통계 · 신규 자료는 산업통상부 공식 PDF에서 수집합니다. 잠정치 및 수정치가 포함됩니다.';
 const link=document.createElement('a');link.href=data.report.policyUrl;link.textContent=data.report.title;
 $('report-source').replaceChildren('최신 공식 자료: 산업통상부 ',link);
 $('source').textContent=data.mode==='sample'?'화면 확인용 가상 데이터 · KOSIS 연결 전':`산업통상부 공식 자료 및 과거 보관 자료 · 수집: ${data.updatedAt?.slice(0,10) || '확인 필요'}`;
